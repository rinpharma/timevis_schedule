function(input, output, session) {
  ...
  core_data <- reactive({
    ...
    ## loading schedule -----------
    progress$set(value = 1, message = 'Pulling schedule from gsheet')
    #token <- gs_auth()
    ...
    list(
      "inputdata" = inputdata,
      "timevisDataGroups" = timevisDataGroups,
      "g_table" = g_table
    )
  })
  ...
  output$timelineGroups <- renderTimevis({
    validate(need(!is.null(core_data()),"Waiting on data"))
    timevis(
      data = core_data()$inputdata,
      groups = core_data()$timevisDataGroups,
